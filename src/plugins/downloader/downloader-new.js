...
module.exports = [

  {
    ...
    execute: async ({ sock, msg, chatId, args, reply }) => {
      const url = args[0];
      if (!url || !url.includes('facebook') && !url.includes('fb.watch')) return reply(h.demonError('.facebookreel', '.facebookreel <facebook reel url>'));
      ...
      try {
        ...
        } else { reply(h.demonFail('could not get download link — try a direct reel URL')); }
      } catch (e) { reply(h.demonFail(`Facebook Reel DL failed — ${e.message}`)); }
    }
  },

  {
    ...
    execute: async ({ sock, msg, chatId, args, reply }) => {
      const url = args[0];
      if (!url || !url.includes('facebook') && !url.includes('fb.')) return reply(h.demonError('.fbstory', '.fbstory <public facebook story url>'));
      ...
      try {
        ...
        } else { reply(h.demonFail('no media found — stories must be public')); }
      } catch (e) { reply(h.demonFail(`Facebook Story DL failed — ${e.message}`)); }
    }
  },
...

  {
    ...
    execute: async ({ sock, msg, chatId, reply }) => {
      ...
      if (!videoMsg) return reply(h.demonFail('reply to a video to convert it to MP3'));
      ...
      } catch (e) { reply(h.demonFail(`MP3 conversion failed — ${e.message}`)); }
    }
  },

  {
    ...
    execute: async ({ sock, msg, chatId, args, reply }) => {
      const url = args[0];
      if (!url || !url.includes('.m3u8')) return reply(h.demonError('.m3u8dl', '.m3u8dl <m3u8 stream url>'));
      ...
      try {
        ...
        if (size > 50 * 1024 * 1024) { fs.removeSync(tmpOut); return reply(h.demonFail('output too large to send (>50MB)')); }
        const buf = fs.readFileSync(tmpOut);
        await sock.sendMessage(chatId, { video: buf, caption: `📡 *M3U8 Stream Download*\n\n_𝗖𝗿𝗶𝘁𝘁𝗶𝘅 𝗠𝗗_` }, { quoted: msg });
        fs.removeSync(tmpOut);
      } catch (e) { reply(h.demonFail(`M3U8 download failed — ${e.message}`)); }
    }
  },

  {
    ...
    execute: async ({ text, args, reply }) => {
      ...
      if (!query) return reply(h.demonError('.torrentsearch', '.torrentsearch <title>'));
      ...
      try {
        ...
        if (!data?.length || data[0]?.name === 'No results returned') return reply(h.demonFail(`no torrents found for "${query}"`));
        ...
        reply(txt);
      } catch (e) { reply(h.demonFail(`torrent search failed — ${e.message}`)); }
    }
  },

  {
    ...
    execute: async ({ args, reply }) => {
      const url = args[0];
      if (!url || !url.includes('mega.nz')) return reply(h.demonError('.megadl', '.megadl <mega.nz link>'));
      reply(
...

  {
    ...
    execute: async ({ sock, msg, chatId, args, reply }) => {
      const url = args[0];
      if (!url || !url.includes('gofile')) return reply(h.demonError('.gofiledl', '.gofiledl <gofile.io link>'));
      ...
      try {
        ...
        if (!files) return reply(h.demonFail('could not access Gofile link — may be password protected or expired'));
      ...
      } catch (e) { reply(h.demonFail(`Gofile fetch failed — ${e.message}`)); }
    }
  },
...

  {
    ...
    execute: async ({ sock, msg, chatId, args, reply }) => {
      const url = args[0];
      if (!url || !url.includes('dropbox')) return reply(h.demonError('.dropboxdl', '.dropboxdl <dropbox.com share link>'));
      ...
      try {
        ...
        if (size > 50 * 1024 * 1024) return reply(h.demonFail(`file too large (${(size/1024/1024).toFixed(1)}MB) to send through WhatsApp`));
      ...
      } catch (e) { reply(h.demonFail(`Dropbox DL failed — ${e.message}`)); }
    }
  },

  {
    ...
    execute: async ({ sock, msg, chatId, args, reply }) => {
      const url = args[0];
      if (!url || !url.includes('vimeo')) return reply(h.demonError('.vimeodl', '.vimeodl <vimeo.com/... url>'));
      await reply('⬇️ fetching Vimeo video...');
      try {
        const videoId = url.match(/vimeo\.com\/(\d+)/)?.[1];
        if (!videoId) return reply(h.demonFail('could not extract Vimeo video ID'));
      ...
      } catch (e) { reply(h.demonFail(`Vimeo DL failed — ${e.message}`)); }
    }
  },

  {
    ...
    execute: async ({ sock, msg, chatId, args, reply }) => {
      const url = args[0];
      if (!url || !url.includes('bilibili')) return reply(h.demonError('.bilibilidl', '.bilibilidl <bilibili.com url>'));
      ...
      try {
        ...
        } else { reply(h.demonFail('Bilibili download link not available — try a direct video URL')); }
      } catch (e) { reply(h.demonFail(`Bilibili DL failed — ${e.message}`)); }
    }
  }
